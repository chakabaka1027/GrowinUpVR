...
public class Rotator : MonoBehaviour
{
    ...
    void OnTriggerEnter(Collider col)
    {
        if (col.gameObject.tag == "Player" && !FindObjectOfType<PlayerController>().hasCookie)
        {
            ...
            if (FindObjectOfType<PlayerController>().hasObtainedCookie == false)
            {
                FindObjectOfType<PlayerController>().hasObtainedCookie = true;
                StartCoroutine(FindObjectOfType<PlayerUI>().DisplayTutorialText("Press E to Throw Cookie"));
